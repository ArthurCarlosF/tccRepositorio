...
float sensores[7] = {0, 0, 0, 0, 0, 0,0 }; //Medicoes dos sensores{EIX,EIY,EIZ,EAX,EAY,EAZ,ED}
...
void hcsr04() {
  ...
  sensores[6] = ultrasonic.convert(microsec, Ultrasonic::CM);
...
void lerGiroscopioAcelerometro(){
  ...
  sensores[0]=float(GyX);
  sensores[1]=float(GyY);
  sensores[2]=float(GyZ);
  sensores[3]=float(AcX);
  sensores[4]=float(AcY);
  sensores[5]=float(AcZ);
